...
BLOCK_DATA_MAPPING = {
    ...
    # === 📋 VAKA FORMUNDAN BLOKLAR ===
    "vf_hasta_bilgileri": {
        "tc_no": lambda c, m: c.get("patient", {}).get("tc_no", "") or m.get("tcNo", "") or "",
        "ad": lambda c, m: c.get("patient", {}).get("name", "") or m.get("patientName", "") or "",
        "soyad": lambda c, m: c.get("patient", {}).get("surname", "") or m.get("patientSurname", "") or "",
        "dogum_tarihi": lambda c, m: c.get("patient", {}).get("birth_date", "") or m.get("birthDate", "") or "",
        ...
        "durum": lambda c, m: c.get("patient", {}).get("status", "") or m.get("patientStatus", "") or "",
    },
    "vf_cagri_bilgileri": {
        "cagri_zamani": lambda c, m: c.get("created_at", "").strftime("%d.%m.%Y %H:%M") if hasattr(c.get("created_at", ""), "strftime") else m.get("callTime", "") or "",
        "arayan_kisi": lambda c, m: c.get("caller", {}).get("name", "") or m.get("callerName", "") or "",
        "arayan_telefon": lambda c, m: c.get("caller", {}).get("phone", "") or m.get("callerPhone", "") or "",
        "firma": lambda c, m: c.get("caller", {}).get("company_name", "") or m.get("company", "") or "",
        "aciklama": lambda c, m: c.get("patient", {}).get("complaint", "") or m.get("complaint", "") or "",
    },
    "vf_konum_bilgileri": {
        "il": lambda c, m: c.get("location", {}).get("city", "") or m.get("city", "") or "",
        "ilce": lambda c, m: c.get("location", {}).get("district", "") or m.get("district", "") or "",
        "mahalle": lambda c, m: c.get("location", {}).get("neighborhood", "") or m.get("neighborhood", "") or "",
        "adres": lambda c, m: c.get("location", {}).get("address", "") or m.get("address", "") or "",
        "koordinat": lambda c, m: f"{c.get('location', {}).get('lat', '')}, {c.get('location', {}).get('lng', '')}" if c.get("location", {}).get("lat") else "",
    },
    "vf_vital_bulgular": {
        "saat": lambda c, m: m.get("vitalSigns", [{}])[0].get("time", "") if m.get("vitalSigns") else "",
        "tansiyon": lambda c, m: m.get("vitalSigns", [{}])[0].get("bp", "") if m.get("vitalSigns") else m.get("vitals", {}).get("bp", "") or "",
        "nabiz": lambda c, m: str(m.get("vitalSigns", [{}])[0].get("pulse", "") if m.get("vitalSigns") else m.get("vitals", {}).get("pulse", "") or ""),
        "spo2": lambda c, m: str(m.get("vitalSigns", [{}])[0].get("spo2", "") if m.get("vitalSigns") else m.get("vitals", {}).get("spo2", "") or ""),
        "solunum": lambda c, m: str(m.get("vitalSigns", [{}])[0].get("respiration", "") if m.get("vitalSigns") else m.get("vitals", {}).get("respiratory_rate", "") or ""),
        "ates": lambda c, m: str(m.get("vitalSigns", [{}])[0].get("temp", "") if m.get("vitalSigns") else m.get("vitals", {}).get("temperature", "") or ""),
        "kan_sekeri": lambda c, m: str(m.get("vitals", {}).get("blood_sugar", "") or m.get("bloodSugar", "") or ""),
    },
    "vf_klinik_gozlemler": {
        "bilinc": lambda c, m: m.get("clinical_observations", {}).get("consciousness", "") or "",
        "pupil": lambda c, m: m.get("clinical_observations", {}).get("pupil_response", "") or "",
        "cilt": lambda c, m: m.get("clinical_observations", {}).get("skin_status", "") or "",
        "solunum_tipi": lambda c, m: m.get("clinical_observations", {}).get("breathing_type", "") or "",
        "nabiz_tipi": lambda c, m: m.get("clinical_observations", {}).get("pulse_type", "") or "",
        "motor": lambda c, m: str(m.get("clinical_observations", {}).get("motorResponse", "") or ""),
        "verbal": lambda c, m: str(m.get("clinical_observations", {}).get("verbalResponse", "") or ""),
        "goz": lambda c, m: str(m.get("clinical_observations", {}).get("eyeOpening", "") or ""),
        "gks_toplam": lambda c, m: str(m.get("clinical_observations", {}).get("gcs_total", "") or ""),
    },
    "vf_anamnez": {
        "sikayet": lambda c, m: c.get("patient", {}).get("complaint", "") or m.get("complaint", "") or "",
        "oyku": lambda c, m: m.get("anamnesis", "") or m.get("history", "") or "",
        "kronik": lambda c, m: m.get("chronicDiseases", "") or m.get("chronic_diseases", "") or "",
        "alerji": lambda c, m: m.get("allergies", "") or m.get("allergy", "") or "",
        "mevcut_ilaclar": lambda c, m: m.get("current_medications", "") or m.get("medications", "") or "",
    },
    "vf_fizik_muayene": {
        "genel_durum": lambda c, m: m.get("physical_exam", {}).get("general_status", "") or "",
        "bas_boyun": lambda c, m: m.get("physical_exam", {}).get("head_neck", "") or "",
        "gogus": lambda c, m: m.get("physical_exam", {}).get("chest", "") or "",
        "karin": lambda c, m: m.get("physical_exam", {}).get("abdomen", "") or "",
        "ekstremite": lambda c, m: m.get("physical_exam", {}).get("extremities", "") or "",
        "norolojik": lambda c, m: m.get("physical_exam", {}).get("neurological", "") or "",
    },
    "vf_uygulamalar": {
        "islemler": lambda c, m: ", ".join(m.get("procedures", [])) if isinstance(m.get("procedures"), list) else m.get("procedures", "") or "",
        "damar_yolu": lambda c, m: m.get("iv_access", "") or "",
        "hava_yolu": lambda c, m: m.get("airway_management", "") or "",
        "cpr": lambda c, m: "Evet" if m.get("cpr_performed") else "Hayir",
        "diger": lambda c, m: m.get("other_procedures", "") or "",
    },
    "vf_genel_notlar": {
        "notlar": lambda c, m: m.get("notes", "") or m.get("generalNotes", "") or "",
    },
    "vf_ilaclar": {
        "ilac_listesi": lambda c, m: format_medications(m.get("medications_used", [])),
    },
    "vf_malzemeler": {
        "malzeme_listesi": lambda c, m: format_materials(m.get("materials_used", [])),
    },
    "vf_nakil_bilgileri": {
        "nakil_tipi": lambda c, m: m.get("transfer_type", "") or m.get("transferType", "") or "",
        "sonuc": lambda c, m: m.get("extended_form", {}).get("outcome", "") or m.get("outcome", "") or "",
        "hastane": lambda c, m: m.get("transfer_hospital", {}).get("name", "") or m.get("transfer1", "") or "",
        "hastane_protokol": lambda c, m: m.get("hospital_protocol", "") or m.get("hospitalProtocol", "") or "",
    },
    "vf_arac_bilgileri": {
        "plaka": lambda c, m: c.get("vehicle_info", {}).get("plate", "") or c.get("vehicle", {}).get("plate", "") or "",
        "arac_tipi": lambda c, m: c.get("vehicle_info", {}).get("type", "") or c.get("vehicle", {}).get("type", "") or "",
        "baslangic_km": lambda c, m: str(c.get("vehicle_info", {}).get("start_km", "") or ""),
        "bitis_km": lambda c, m: str(c.get("vehicle_info", {}).get("end_km", "") or ""),
        "protokol_112": lambda c, m: c.get("protocol_112", "") or m.get("protocol112", "") or "",
    },
    "vf_ekip_bilgileri": {
        "sofor": lambda c, m: c.get("team", {}).get("driver", {}).get("name", "") or "",
        "paramedik": lambda c, m: c.get("team", {}).get("paramedic", {}).get("name", "") or "",
        "att": lambda c, m: c.get("team", {}).get("att", {}).get("name", "") or "",
        "hemsire": lambda c, m: c.get("team", {}).get("nurse", {}).get("name", "") or "",
        "doktor": lambda c, m: c.get("team", {}).get("doctor", {}).get("name", "") or "",
    },
    "vf_onam_bilgilendirme": {
        "onam_metni": lambda c, m: m.get("inline_consents", {}).get("patient_info", {}).get("text", "") or "Hasta bilgilendirilmistir.",
        "hasta_imza": lambda c, m: "[IMZA]",
        "tarih": lambda c, m: datetime.now().strftime("%d.%m.%Y"),
    },
    "vf_hastane_reddi": {
    ...
    "vf_hasta_reddi": {
        "red_nedeni": lambda c, m: m.get("inline_consents", {}).get("patient_rejection", {}).get("reason", "") or "",
        "hasta_adi": lambda c, m: m.get("inline_consents", {}).get("patient_rejection", {}).get("patient_name", "") or "",
        "yakini_adi": lambda c, m: m.get("inline_consents", {}).get("patient_rejection", {}).get("relative_name", "") or "",
        "imza": lambda c, m: "[IMZA]",
    },
    "vf_teslim_imzalari": {
        "teslim_alan": lambda c, m: m.get("inline_consents", {}).get("handover", {}).get("receiver_name", "") or "",
        "doktor_imza": lambda c, m: "[IMZA]",
        "saglik_personeli_imza": lambda c, m: "[IMZA]",
        "sofor_imza": lambda c, m: "[IMZA]",
    },
    "vf_tarihce": {
        "olusturma": lambda c, m: c.get("created_at", "").strftime("%d.%m.%Y %H:%M") if hasattr(c.get("created_at", ""), "strftime") else "",
        "guncelleme": lambda c, m: c.get("updated_at", "").strftime("%d.%m.%Y %H:%M") if hasattr(c.get("updated_at", ""), "strftime") else "",
        "durum": lambda c, m: c.get("status", "") or "",
    },
    # === 🆕 YENİ PDF BLOKLARI ===
