...
def excel_form_to_pdf_with_data(
    ...
    if case_data or form_data:
        ...
        cell_mapping = {
            # Header section - ATN NO (label at T3:V4, INPUT at T5:V7)
            'T5': case_data.get('case_number', ''),  # ATN NO input - CORRECTED!
            ...
            # HASTANIN ŞİKAYETİ (label at Y11:AB11, need to find input - AC11 is outside the merge)
            'AC11': form_data.get('complaint') or safe_get(case_data, 'patient', 'complaint') or case_data.get('description', ''),  # ŞİKAYET input
        }
